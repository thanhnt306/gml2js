  ...
  <div class="w-[345px] flex flex-col items-center">
    ...
    <form @submit.prevent="handleSubmit" class="w-full space-y-4">
      ...
      <div v-if="currentState === 'signUp'" class="flex flex-col space-y-2">
        ...
        <div class="relative">
          ...
          <img src="@/assets/images/email.png"
            class="absolute right-3 top-1/2 transform -translate-y-1/2 w-[17px] h-[17px] opacity-70" />
        </div>
      </div>
      ...
      <div v-if="currentState === 'signInCompany'" class="flex flex-col space-y-2">
        ...
        <input ref="companyInput" v-model="form.company" type="text" placeholder="Enter your company code"
          class="w-full h-10 bg-transparent border border-[#5D5D5D] rounded-lg px-3 text-[#bebebe] placeholder-[#5D5D5D] focus:outline-none focus:border-white transition-colors"
          required />
      </div>
      ...
      <div v-if="['signIn', 'signUp'].includes(currentState)" class="flex flex-col space-y-2">
        <label class="text-white font-montserrat font-medium text-sm">User name</label>
        <div class="relative">
          <input v-model="form.username" type="text" placeholder="Enter your username"
            :disabled="currentState === 'signUp' && isAdminRequired"
            :class="{'opacity-50 cursor-not-allowed': currentState === 'signUp' && isAdminRequired}"
            class="w-full h-10 bg-transparent border border-[#5D5D5D] rounded-lg px-3 pr-10 text-[#bebebe] placeholder-[#5D5D5D] focus:outline-none focus:border-white transition-colors" />
      ...
      <div v-if="currentState === 'signIn'" class="flex justify-between pt-1">
        <button @click="currentState = 'signInCompany'" type="button"
          class="text-[#A7A7A7] text-sm hover:text-white font-montserrat font-semibold">
          &larr; Back
        </button>
      ...
      <div v-if="currentState === 'activateLicense'" class="flex flex-col space-y-2">
        ...
        <div class="relative">
          <input v-model="form.licenseKey" type="text" placeholder="Enter your License key"
            class="w-full h-10 bg-transparent border border-[#5D5D5D] rounded-lg px-3 pr-10 text-[#bebebe] placeholder-[#5D5D5D] focus:outline-none focus:border-white transition-colors" />
...
const isAdminRequired = ref(false)
...
const submitButtonText = computed(() => {
  switch (currentState.value) {
    case 'signInCompany': return 'Sign in'
...
const toggleState = async (): Promise<void> => {
  if (currentState.value === 'signInCompany' || currentState.value === 'signIn') {
    if (currentState.value === 'signInCompany' && !form.company.trim()) {
      companyInput.value?.reportValidity()
      return
    }

    isSubmitting.value = true
    const hasAdmin = await authStore.checkAdminExists(form.company)
    isSubmitting.value = false
    
    if (!hasAdmin) {
      form.username = 'admin'
      isAdminRequired.value = true
    } else {
      if (isAdminRequired.value) form.username = '' // Reset if previously blocked
      isAdminRequired.value = false
    }

    currentState.value = 'signUp'
  } else {
    currentState.value = 'signInCompany'
  }
}
  ...
  try {
    if (currentState.value === 'signIn') {
      ...
      if (success) {
        ...
        currentState.value = 'signIn' // shift to Username/Password
      } else {
